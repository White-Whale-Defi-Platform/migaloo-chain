...
func TestIBCFeesTransfer(t *testing.T) {
	...
	coord.Setup(path)
	appA, ok := chainA.App.(*app.MigalooApp)
	if !ok {
		t.Fatalf("Expected *app.MigalooApp, got %T", coord.GetChain(wasmibctesting.GetChainID(1)).App)
	}
	...
	require.NoError(t, err)
	appB := chainB.App.(*app.MigalooApp)
...
func TestIBCFeesWasm(t *testing.T) {
	...
	coord.Setup(path)
	appA := chainA.App.(*app.MigalooApp)
	appB := chainB.App.(*app.MigalooApp)
